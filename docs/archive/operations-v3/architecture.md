...
| `AUTH_ENCRYPTION_[REDACTED]` | (derived) | ContextForge encryption key. Derived by entrypoint.sh from JWT key. Not set in cloudbuild.yaml — set at runtime only. |
| `PLATFORM_ADMIN_[REDACTED]` | (derived) | ContextForge admin credential. Derived by entrypoint.sh from AUTH key. Not set in cloudbuild.yaml — set at runtime only. |
